...
# Define the conditions for bullish_4c entries
def is_bullish_4c_entry(df, index):
    if index < 4:
        return False
    
    high1 = df.at[index, 'High']
    high2 = df.at[index-1, 'High']
    high3 = df.at[index-2, 'High']
    high4 = df.at[index-3, 'High']
    close1 = df.at[index, 'Last']
    open1 = df.at[index, 'Open']
    open4 = df.at[index-3, 'Open']
    low1 = df.at[index, 'Low']
    low2 = df.at[index-1, 'Low']
...
for index, row in df.iterrows():
    if is_bullish_4c_entry(df, index):
...
trades_df.to_csv(r"D:\data\60 min.csv", index=False)
